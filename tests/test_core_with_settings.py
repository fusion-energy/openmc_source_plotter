...
def test_sample_initial_particles(test_settings):
    particles = sample_initial_particles(this=test_settings, n_samples=43)
...
def test_energy_plot(test_settings):
    plot = plot_source_energy(this=test_settings, n_samples=10)
    assert isinstance(plot, go.Figure)
    assert len(plot.data[0]["x"]) == 1


def test_position_plot(test_settings):
    plot = plot_source_position(this=test_settings, n_samples=10)
    assert isinstance(plot, go.Figure)


def test_direction_plot(test_settings):
    plot = plot_source_direction(this=test_settings, n_samples=10)
    assert isinstance(plot, go.Figure)


def test_energy_plot_with_figure(test_settings):
    base_figure = go.Figure()
    plot = plot_source_energy(this=test_settings, figure=base_figure, n_samples=10)
    assert isinstance(plot, go.Figure)
    assert len(plot.data[0]["x"]) == 1
...
def test_position_plot_with_figure(test_settings):
    base_figure = go.Figure()
    plot = plot_source_position(this=test_settings, figure=base_figure, n_samples=10)
    assert isinstance(plot, go.Figure)


def test_direction_plot_with_figure(test_settings):
    base_figure = go.Figure()
    plot = plot_source_direction(this=test_settings, figure=base_figure, n_samples=10)
    assert isinstance(plot, go.Figure)
